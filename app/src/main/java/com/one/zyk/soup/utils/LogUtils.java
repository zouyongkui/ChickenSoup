...
public final class LogUtils {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = SoupApp.getInstance()
                    .getPackageManager()
                    .getPackageInfo(SoupApp.getInstance().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && SoupApp.getInstance().getExternalCacheDir() != null)
                sDefaultDir = SoupApp.getInstance().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = SoupApp.getInstance().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
